...
% overall transition entropy before and after replacing states w/ chi sq analysis
%% plotting%% transition entropy pre vs change in entropy
...
[correlationpreTE,pvalpreTE]=corr(tebpdiff_all,tebp(:,1));
...
f3=figure('Name','SIGNS_TEBP_pre_deltaTEBP')
scatter(tebp_bird1(:,1),tebp_bird1_diff,'o',...
              'LineWidth',1.5)
hold on
scatter(tebp_bird2(:,1),tebp_bird2_diff,'+',...
              'LineWidth',1.5)
scatter(tebp_bird3(:,1),tebp_bird3_diff,'x',...
               'LineWidth',1.5)
scatter(tebp_bird4(:,1),tebp_bird4_diff,'s',...
              'LineWidth',1.5)
scatter(tebp_bird5(:,1),tebp_bird5_diff,'d',...
              'LineWidth',1.5)
scatter(tebp_bird6(:,1),tebp_bird6_diff,'^',...
               'LineWidth',1.5)
scatter(tebp_bird7(:,1),tebp_bird7_diff,'h',...
               'LineWidth',1.5)
xlim([0 3])
h=refline(0,0)
%h=refline(1,0)
...
title('Transition entropy prelesion vs change in transition entropy','(n=56 branchpoints in n=7 birds)')
xlabel('Entropy prelesion')
ylabel('Change in entropy')
hold off
% testing significance with one sided ttest
tebp_diff_vector=[tebp_bird1_diff;tebp_bird3_diff;tebp_bird4_diff;tebp_bird5_diff;tebp_bird6_diff;tebp_bird7_diff];
[httest,pttest,cittest,statsttest]=ttest(tebp_diff_vector);
